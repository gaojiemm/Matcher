...
`.github/workflows/node24-action-demo.yml` ではこの 3 つのローカル action を実際に呼び出して出力まで確認できます。

## Node ランタイムのアップグレード方法

ランタイムを別バージョンへ更新する場合は、次のコマンドを実行するだけです。AI がファイルの読み書きから確認まですべて行います。
...
更新の手順と確認項目は `.github/prompts/node-runtime-upgrade.prompt.md` に記述されています。

## upgrade の検証について

アップグレード後の確認は `copilot -p` コマンドの実行時に AI が自動で行います。結果は Markdown の表で報告されます。手動でスクリプトを実行する必要はありません。
